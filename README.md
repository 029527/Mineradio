...
一键安装（自动拉取最新版、装入 `/Applications`、去掉隔离属性后启动）：

```bash
/bin/bash -c "$(curl -fsSL https://raw.githubusercontent.com/029527/Mineradio/main/install.sh)"
```

或手动从 [Releases](https://github.com/029527/Mineradio/releases) 下载 `Mineradio_<版本>_aarch64.dmg`，
拖入「应用程序」。应用未签名，若提示「无法打开」，右键图标 → 打开，或执行：

```bash
xattr -dr com.apple.quarantine /Applications/Mineradio.app
```

> 目前仅提供 Apple 芯片（arm64）版本；Intel Mac 暂不支持。
